...
export class MarkComponent implements OnInit {
  ...
  imgPath2 = '';
  imgPath3 = '';
  markCanvas2Display = 'none';
  markCanvas3Display = 'none';
  ...
  updateCanvas(): void {
    ...
    for (const group of this.mark.groups) {
  ...
  updateScoreBoard(): void {
    this.scoreList = [];
    const score = Number(this.fullScore);
    const unit = parseFloat(this.scoreUnit);
    for (let i = 0; i <= score; i += unit) {
    ...
    for (const group of this.mark.groups) {
      if (group.groupNo === this.curPage) {
        if (group.papers[0].score !== '') {
          ...
          for (const step of group.papers[0].steps) {
        ...
        if (this.markQuestions.length >= 2) {
          if (group.papers[1].score !== '') {
            ...
            for (const step of group.papers[1].steps) {
        ...
        if (this.markQuestions.length == 3) {
          if (group.papers[2].score !== '') {
            ...
            for (const step of group.papers[2].steps) {
